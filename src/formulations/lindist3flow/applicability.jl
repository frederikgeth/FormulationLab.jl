...
const _L3F_UNSUPPORTED_FAMILIES = (
    ...
    "dc_bus", "dc_line", "dc_branch", "dc_grounding", "dc_load", "dc_source", "dc_converter", "time_series",
)
...
function _l3f_input(input)
    if input isa BMOPFInput
        return _normalize_bmopf_snapshot(input.network)
    elseif input isa PowerIO.PioModule
        return _normalize_bmopf_snapshot(read_bmopf(input).network)
    elseif input isa AbstractString
        return _normalize_bmopf_snapshot(read_bmopf(input).network)
    elseif input isa AbstractDict
        return _normalize_bmopf_snapshot(input)
    end
