...
Ratings are computed directly from physical input. Restating them explicitly
keeps source and physical transformer endpoint scaling independent of the
internal working-copy representation.
"""
...
function _l3f_working_coordinates(net, reference::L3FReferenceState,
    ...
    working, bases = _l3f_scale_network(net, options.s_base)
...
"""
    l3f_reference_from_powerflow(net; options=L3FOptions(), dispatch=nothing,
                                 nonlinear_optimizer=nothing, powerflow=nothing,
...
better = l3f_reference_from_powerflow(net; options, dispatch=first, powerflow=my_powerflow)
second = solve_l3f_opf(net, Clarabel.Optimizer; options, reference=better)
```

A reference power flow is a determined problem, so this helper requires every generator to
...
function l3f_reference_from_powerflow(net; options::L3FOptions=L3FOptions(),
    ...
    powerflow === nothing && throw(ArgumentError("Pass powerflow=(network; kwargs...) -> result to construct a nonlinear reference."))
    pf = powerflow(working; optimizer=nonlinear_optimizer, solver_options)
...
"""
    validate_l3f_solution(result; nonlinear_optimizer=nothing, powerflow=nothing,
                          voltage_tolerance=nothing)

Reapply the L3F generator dispatch to `result.network` — the SI, Kron-reduced
snapshot the model was built from — and run the supplied `powerflow` callback.
Without a callback, validation reports `unavailable`.
...
function validate_l3f_solution(result::L3FResult;
    ...
    powerflow === nothing && return Dict{String,Any}("status" => "unavailable",
        "reason" => "No powerflow callback supplied", "physical_limits" => "unassessed")
    ...
    try
        pf = powerflow(working; optimizer=nonlinear_optimizer)
        termination = String(get(pf, "termination_status", "UNKNOWN"))
...
function solve_l3f_opf(net, optimizer=default_optimizer();
    ...
    if options.validate_nonlinear
        ...
        validation = unavailable === nothing ?
            validate_l3f_solution(result; nonlinear_optimizer, powerflow, voltage_tolerance) :
