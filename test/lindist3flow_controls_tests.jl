...
@testset "LinDist3Flow restricted controls" begin
    @testset "native IBR lowering preserves input and signed PF" begin
        ...
        net = deepcopy(original); findings = FormulationLab.L3FFinding[]
        FormulationLab._l3f_lower_restricted_controls!(findings, net)
    ...
    @testset "unsupported control laws are rejected rather than ignored" begin
        ...
        findings = FormulationLab.L3FFinding[]
        FormulationLab._l3f_lower_restricted_controls!(findings, net)
    ...
    @testset "aggregate and hard-target validation" begin
        valid = Dict{String,Any}("generator" => Dict("g" => Dict{String,Any}(
            ...
            "aggregate_s_max" => 8.0)))
        findings = FormulationLab.L3FFinding[]
        FormulationLab._l3f_validate_restricted_controls!(findings, valid)
        @test isempty(findings)
        invalid = deepcopy(valid)
        invalid["generator"]["g"]["p_max"] = [6.0]
        FormulationLab._l3f_validate_restricted_controls!(findings, invalid)
    ...
    @testset "permissive neutral current handling uses declared role" begin
        net = Dict{String,Any}(
            ...
            "ibr" => Dict("x" => Dict{String,Any}(
                ...
                "i_max" => [10.0, 5.0, 20.0])))
        findings = FormulationLab.L3FFinding[]
        FormulationLab._l3f_permissive_restricted_controls!(findings, net)
        ...
        findings = FormulationLab.L3FFinding[]
        FormulationLab._l3f_permissive_restricted_controls!(findings, malformed)
