...
`per_unit=false` builds the identical formulation in raw SI. FormulationLab uses
FormulationLab's standalone numerical scaling to scale a private working copy,
...
!!! note "The BMOPF regulator spec table has the ANSI mapping backwards"
    ...
    pairing and should be corrected upstream. FormulationLab follows the
    implementation.
...
delta-delta — have no BMOPF component, so FormulationLab reads them under
...
generator dispatch in the reduced snapshot and calls the explicitly supplied `powerflow` callback. Without a callback,
validation reports `unavailable`. The
...
on every build; both coordinate modes enforce the same surrogate limits.
