...
This page is the normative mathematical specification of FormulationLab's
...
`L3FOptions(per_unit=true, s_base=S_b)` is the default. FormulationLab's standalone
numerical scaling supplies the working copy and bases
...
Voltage-source apparent-power and current ratings are scaled directly from SI.
Regression tests pin their equivalence in both coordinate modes.
...
modes: FormulationLab scales `cost` by ``S_b`` when it builds the per-unit working
...
4. If requested, use FormulationLab's standalone numerical scaling to create a
   per-unit working copy and bases.
5. Build the independent FormulationLab affine/SOC JuMP model.
6. Extract all published voltages and powers back to SI; optionally replay the
   fixed dispatch with the caller-supplied `powerflow` callback.
