...
| Fields / behavior | LinDist3Flow | BranchFlowSDP v1 | Dense IVRSDP / IVRSOC |
|---|---|---|---|
...
| Load `p_nom`, `q_nom`, connections | Fixed-reference channel allocation | Ground-referenced WYE/single-phase channels | Physical incidence and channel currents |
...
| Transformers/regulators | Single-phase, center-tap, Yd/Dy, autotransformer, open-delta; documented restrictions/lowering | Unsupported | All seven subtypes, including general multiwinding; internal grounding and explicit excitation |
| `tap_ratio*` on ordinary transformers | Normalized to migrated `tap*` spelling; conflicts rejected | Unsupported | Fixed setting or equal bounds; adjustable optimization rejected |
...
The branch-flow column is the conservative `:radial_series_v1` prototype scope,
not the intended final coverage. See the [branch-flow equations, refusals and
extension order](branch_flow_sdp.md).
